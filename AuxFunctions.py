...
class AuxFunctions:
    ...
    def __init__(self):
        global log_utility
        if log_utility is None:
            log_utility = LogUtility ("C:\\temp\\bg_log.log")
    def Log(self):
        return log_utility
    ...
    def TakeBoardSnapshot(self):
        folder = "C:\\board_snapshot"
        
        filename = folder + "\\" + str(self.FindMissingNumber(folder,"jpg"))+".jpg"
        cap=cv2.VideoCapture(0)
        ret,image=cap.read()
        cv2.imwrite(filename, image)
        cap.release()
    ...
    def mean_grey_value_from_camera(self,image):

        # Convert the frame to grayscale
        grey_img = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

        # Calculate the mean value of the pixel intensities
        mean_value = np.mean(grey_img)
...
# Example usage
#af = AuxFunctions()
#mean_value = af.mean_grey_value_from_camera()
#print(f"The mean grayscale value of the captured image is: {mean_value}")
