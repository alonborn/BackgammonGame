...
class DataModel:
    ...
    adjust_maneuver_height_value = -7
    ...
    def CreateGameStringForGNUBG2(self,switch:bool):
        #GNU is always playing white. so if we want to play instead of human player- switch = True and then the function switches 
        # between white and black - it mirrors the board. later on, movement should be adjusted
        pieces1 = self.black_pieces
        pieces2 = self.white_pieces

        if switch:#switch black and white, to play instead of human
            pieces2 = self.black_pieces
            pieces1 = self.white_pieces


        board_string = "set board simple "
        board_string += str(pieces2.count(0) if switch else pieces2.count(25)) + " " #white bar. if switch - it should contain black bar

        #for i in range(24,0,-1):
        start, end, step = (1, 25, 1) 
        if switch== True:
            start, end, step = (24, 0, -1)

        for i in range(start, end, step):
            if pieces1.count(i) > 0 :
                board_string += "-" +str(pieces1.count(i))
            elif pieces2.count(i) > 0:
                board_string +=  str(pieces2.count(i))
            else:    
                board_string += "0" 
            board_string += " "

        board_string += str(pieces1.count(25) if switch else pieces1.count(0)) + " "    # black bar.if switch - it should contain white bar 
    ...
    def GetDiceModelName(self):
        if self.phone == "Dino":
            return "dice_recognizer_model_v3_dino.pth"
        if self.phone == "BP":
            return "dice_recognizer_model_BP.pth"