...
const int PIN_SPI_SLAVE_SELECT = 10; // Slave Select pin
const int PIN_SPI_SLAVE_IS_READY = 8;


int magnet_servo_target_angle = 0;   //to which angle the servo needs to rotate to
int magnet_speed = 0; //1 --> slowly 2 --> rapidly
int magnet_servo_start_rotation = 0;  //from which x or y to start rotate the magnet servo
...
#define CHUNK_SIZE 28      // Define the chunk size
...
{
  pinMode(PIN_SPI_SLAVE_SELECT, OUTPUT); // The CS_Pin should remain in a high impedance state (INPUT) when it is not in use.
  digitalWrite(PIN_SPI_SLAVE_SELECT, LOW); // Activate the CS line (CS is active LOW)
  bool slaveReady = false;
  while (!slaveReady) //wait until slave is ready
  {
    slaveReady = digitalRead(PIN_SPI_SLAVE_IS_READY) == HIGH;
    delay(10);
  }

  for (int i = 0 ; i < MAX_STRING_LENGTH-1 ; i++)
  {
    SPI.transfer(message[i]); // Send first byte // DEC 36
  }

  pinMode(PIN_SPI_SLAVE_SELECT, INPUT); // Set CS_Pin to high impedance to allow pull-up to reset CS to inactive.
  digitalWrite(PIN_SPI_SLAVE_SELECT, HIGH); // Enable internal pull-up
  SerialPrintln("Sent to matrix:" + message);
}
...
void GetMoveAndRotateParams(String move_command, String magnet_line)
{
  ...
  magnet_servo_start_rotation = GetSectionByIndex(magnet_line,5,':').toInt();  
