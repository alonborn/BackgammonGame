...
class BoardController:
    ...
    CALIBRATION_POS_Y = 220
       #columns in the 3rd and 4th quarter are a bit closer to the magnet, so it needs to adjust maneuver height
    ...
    def GetMagnetManeuverCommand(self,adjust_maneuver_height=0):
        if self.data_model.skip_magnet == True:
            return ''
        return "Magnet:1:"+ str(self.magnet_angle_maneuver + adjust_maneuver_height)   

    def GetMagnetHoverCommand(self,from_col):
        if self.data_model.skip_magnet == True:
            return ''
        adjust_maneuver_height = 0
        if 12 <= from_col <=23:
            adjust_maneuver_height = self.data_model.adjust_maneuver_height_value  
        return "Magnet:1:"+ str(self.magnet_angle_maneuver+adjust_maneuver_height)     #hover is like maneuver without checker attached
    ...
    def AreColumnsColiding(self,cur_pos,origin,target):
        ...
        checker_rad = self.data_model.CHECKER_RADIUS / self.camera.mmperpixel[0]
        target_point = self.camera.CalculateManeuverPoint(cur_pos,target,0,1)
    ...
    def GetPickHoverCommand(self,target_scr,start_from,allow_move_rotate,from_col):
        start_servo_from = 0
        ...
        adjust_maneuver_height = self.AdjustManeuverHeight(from_col)
        command = []
        ...
        if allow_move_rotate and (start_servo_from != None and (start_from_mach[0] < start_servo_from < target_mach_coords[0] or
            ...
            command.append(self.GetMoveAndRotateCommand(move_command,self.magnet_angle_maneuver + adjust_maneuver_height,"X",start_servo_from,2))
        else:   #if there's no point to do both together
            command.append(move_command)
            
            #print ("adjusting put maneuver")
            command.append(self.GetMagnetManeuverCommand(adjust_maneuver_height))

        return command

    def AdjustManeuverHeight(self, from_col):
    ...
    def CalculateDescendingPointAfterBar(self,cur_pos,to_col):
        ...
        end_point = self.camera.CalculateManeuverPoint(cur_pos,to_col,0,1)
    ...
    def GetMoveToPutManeuverCommand(self,from_col,to_col,cur_pos, keep_magnet_on):
        ...
        put_maneuver_point = self.camera.CalculateManeuverPoint(cur_pos,to_col,0,1)
        ...
        adjust_maneuver_height = self.AdjustManeuverHeight(to_col)
        if should_descent:
        ...
        if keep_magnet_on == False:
            put_maneuver_point2 = self.camera.CalculateManeuverPoint(cur_pos,to_col,1,1)
    ...
    def IsRetreatNeeded(self,from_col):
        if from_col in (0,5,6,11,12,17,18,23):
            return True
        #if len(self.data_model.all_checkers[from_col]) == 1:   #removed these lines, as retreat is always required #aug 23
        #    return True
        #return False
        return True

    def IsSimpleChecker(self,from_col): #is it a regular checker that needs to be moved a bit before lifting
        if from_col in (0,5,6,11,12,17,18,23):
            return False
        if len(self.data_model.all_checkers[from_col]) == 1:   #removed these lines, as retreat is always required #aug 23
            return  False
    ...
    def PickupChecker(self, from_col,start_from,color):
        ...
        if pick_maneuver_point != None:   #if yes, go to maneuver before picking the checker
            new_command = self.GetPickHoverCommand(pick_maneuver_point,start_from,True,from_col)
            ...
            """next_command = self.GetMagnetHoverCommand(from_col)      #no need to add magnet, since GetPickHoverCommand already does it
            if next_command !="":
                command.append(next_command)"""
            command.append(self.GetMoveToCommand(checker_pos_mach,self.FEED_AT_MANEUVER))
        else:
            command.extend(self.GetPickHoverCommand(checker_pos_scr,start_from,False,from_col))
        ...
        if self.IsRetreatNeeded(from_col):
            ...
            if 12 <= from_col <=23:
                adjust_maneuver_height = self.data_model.adjust_maneuver_height_value    
            next_command = self.GetMagnetManeuverCommand(adjust_maneuver_height)
            if next_command != '':
                command.append(self.GetMagnetManeuverCommand(adjust_maneuver_height))   #need to go up a bit, before retreat

            if from_col in (0,5,6,11,12,17,18,23):
                retreat_pos = self.camera.CalculateManeuverPoint(checker_pos_scr,from_col,1,0)  #check if it needs to 'retreat' to a safe position before lifting up the checker
                ...
                retreat_pos2 = self.camera.CalculateManeuverPoint(retreat_pos,from_col,0,0)
            ...
            #elif len(self.data_model.all_checkers[from_col]) == 1: #retreat for all checkers   #Aug 23
            else:
                retreat_pos = self.camera.CalculateManeuverPoint(checker_pos_scr,from_col,1,0)  #check if it needs to 'retreat' to a safe position before lifting up the checker
                if retreat_pos is None:
                    retreat_pos = self.camera.CalculateManeuverPoint(checker_pos_scr,from_col,0,0)  #check if it needs to 'retreat' to a safe position before lifting up the checker                    
                retreat_mach_pos = self.camera.GetMachineCoords(retreat_pos)
                command.append(self.GetMoveToCommand(retreat_mach_pos))
    ...
    def SendCommandToRGB(self,mode,text1,text2="NONE",pos1 = 0, pos2=0,dice1 = 0, dice2= 0):
        ...
        self.arduino.SendCommand(af.ConcatenateCommand(command),True)

    def SendHomeCommand(self):
        if self.data_model.skip_home == True:
            return
    ...
    def RemoveConsecutiveMagnets(self,arr):
        # Iterate over the array from the end to the beginning to avoid index shifting issues
        i = len(arr) - 2
        while i >= 0:
            if arr[i].startswith('Magnet:1:') and arr[i + 1].startswith('Magnet:1:'):
                del arr[i]
            i -= 1
        return arr
    ...
    def DoAllMovements(self,movements,color, skip_bar):
        ...
        for i,movement in enumerate(movements):
            if movement[1] != -1:
                ...
                commands = self.RemoveConsecutiveMagnets(commands)
        return commands
