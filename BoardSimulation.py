...
class BoardSimulation:
    ...
    def SplitMovement(self,image,cur_pos,end_pos,new_servo_angle_code,magnet_starts_at,servo_angle_code):
        ...
        cur_pos_mach = (round(cur_pos_mach[0], 2), round(cur_pos_mach[1], 2))
        end_pos_mach = (round(end_pos_mach[0], 2), round(end_pos_mach[1], 2))

        mid_pos = 0

        if split_direction == 'X':
            if (cur_pos_mach[0] <=split_point <= end_pos_mach[0]) or (end_pos_mach[0] <= split_point <= cur_pos_mach[0]):
                split_point_y_mach = af.FindYForX(cur_pos_mach,end_pos_mach,split_point)
            else:
                split_point_y_mach = cur_pos_mach[1] if (abs(cur_pos_mach[0] - split_point) < abs(end_pos_mach[0] - split_point)) else end_pos_mach[1]
                print ("split point is not within the start end boundaries")
    ...
    def GetAngleCode(self,angle):
        ...
        if angle in (self.board.magnet_angle_maneuver,self.board.magnet_angle_maneuver-1,self.board.magnet_angle_maneuver + self.data_model.adjust_maneuver_height_value) : #Red
