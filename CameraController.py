...
class CameraController:
    ...
    points_area = [[[500, 10],[570, 104]],[[1125, 10],[1230, 108]],[[1130, 640],[1240, 720]]]
    
    #fixed points
    point_mach =  [[9.62,0.27],[1.04,0.31],[1.13,8.47],[9.68,8.44]]
    ...
    def CalibrateBoard(self):
        ...
        for i in range (2):
            ...
            while (abs(test_calculated[0] - test_actual[0]) > 1):
                ...
                delta = 0.001 if abs(test_calculated[0] - test_actual[0]) > 2 else small_delta
            ...
            while (abs(test_calculated[1] - test_actual[1]) > 1):
                delta = 0.001 if abs(test_calculated[1] - test_actual[1]) > 2 else small_delta
    ...
    def IsNextToBar (self,col):
        return True if col in (0,5,6,11,12,17,18,23) else False


    #calculate landing spot to start manouver from, for PUTTING checkers
    #step 0 - landing spot, far away from other checkers, to avoid collision
    #step 1- only in case of neighbours that can be colided with- go to the middle of the column
    #maneuver_type - 0 - pick, 1 - put
    def CalculateManeuverPoint(self,from_pos,to_col,step,maneuver_type):
        ret_val=[0,0]
        ...
        small_maneuver_needed = self.board.IsSimpleChecker(to_col)   #when picking up a checker that is not the first, still we want to move it a bit before picking up to avoid lifting the checker next to it
        

        put_maneuver_spots = self.GetNeighboursCheckerNextSpot(to_col)    #positions of 'next spots' in the column from both sides
        ...
        if small_maneuver_needed and step == 1 and maneuver_type ==0:   #if only small maneuver is needed (required in step 1 only), just take the position of the last checker and move it a bit Aug 23
            col_next_spot = self.data_model.GetLastChecker(to_col)
        else:
            col_next_spot = self.GetNextSpot(to_col) #next spot for the 'to_col' column

        #adjusting the y coordinate (adding few mm tolerance)
        if af.GetColumnSide(to_col) == 1:    #top side
            col_next_spot = (col_next_spot[0],col_next_spot[1] +y_tolerance)
        else:
            col_next_spot = (col_next_spot[0],col_next_spot[1] -y_tolerance)
        #set x pos as the mid between surrounding columns (before and after) and to_col
        ...
        #setting y coordinate to be the max between to_col and the ones before and after
        ...
        if af.GetColumnSide(to_col) == 1:    #top side
            ...
            put_maneuver_spots = [[item[0], [item[1][0], -3000] if item[1][1] == 3000 else item[1]] for item in put_maneuver_spots] ##BUG!! this is missing from the other part of the 'if' statement
        ...
        if abs(put_maneuver_spots[0][1][1]- put_maneuver_spots[1][1][1]) < magnet_rad:  #spots are relatively close, take the one that is closest to cur_position:
            ...
            if maneuver_type == 1: #put
                ret_val = af.FindClosestPoint(col_next_spot,put_maneuver_spots)
            else:   #pick
                """if self.IsNextToBar(to_col):
                    ret_val = af.FindClosestPoint(col_next_spot,put_maneuver_spots)
                else:"""
                for sublist in put_maneuver_spots:    #take care of the edges (avoid taking them into account in the min/max computation below)
                    sublist[1] = [-3000 if x == 3000 else 3000 if x == -3000 else x for x in sublist[1]]
                #take the safest y value (to avoid collision with other checkers)
                highest_y_value = max(put_maneuver_spots[0][1][1],put_maneuver_spots[1][1][1]) if af.GetColumnSide(to_col) == 1 else min(put_maneuver_spots[0][1][1],put_maneuver_spots[1][1][1])
                ret_val = [col_next_spot[0],highest_y_value]
        
        if self.IsNextToBar(to_col) and maneuver_type == 0: #pick
            if step == 0:
                ret_val = af.FindClosestPoint(col_next_spot,put_maneuver_spots)
            return ret_val
        ...
        if step == 1:
            if ret_val[1] != col_next_spot[1]:
                return [col_next_spot[0],ret_val[1]] #return the center of the to_col position
            else:
                return None
    ...
    def GetNextSpotOnBar(self):
        ...
        distance_between_checkers_on_bar = 7 #mm

        tolerance = (distance_between_checkers_on_bar*self.mm_tolerance) / self.mmperpixel[1]
