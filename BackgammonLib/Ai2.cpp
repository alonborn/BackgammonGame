...
AiConfig* GetAi(int id) {
	return &AIs[id];
}

void SetAi(int id, AiConfig* Ai)
{
	AIs[id] = *Ai;
}
...
bool IsMoveValid(Move move, Game* g)
{
	int from = move.from;
	int to = move.to;
	...
	if (from > to)
		return false;
	if (!(g->Position[from] & move.color))
		return false;
	int otherColor = OtherColor(move.color);
	if (g->Position[to] & otherColor && CheckerCount(g->Position[to]) > 1)
		return false;
	return true;
}
...
bool DoMove(Move move, Game* g) {
	...
	PlayerSide other = (PlayerSide)OtherColor((PlayerSide)move.color);
...
void SetLightScore(Game* g, MoveSet* moveSet) {
	...
	for (int i = 0; i < moveSet->Length; i++)
	{
		...
		if (CheckerCount(g->Position[to]) == 1)
			score -= (int)GetAi(a)->BlotFactors[to];
	}
	moveSet->score = score;
}
...
void CreateMoves(Game* g) {
	...
	int diceCount = g->Dice[0] == g->Dice[1] ? GetSettings()->DiceQuads : 2;
	...
	for (int i = 1; i < 25; i++)
	{
		...
		if (checkCount > 1 && (v & color)) {
			...
			if (blockCount && !playersPassed) {
				score += pow((double)blockCount, GetAi(ai)->ConnectedBlocksFactor[p]);
			}
			blockCount = 0;
		}
...
int RecursiveScore(Game* g, int depth, int best_black, int best_white) {
	...
	MoveSet* localSets = (MoveSet*)malloc(sizeof(MoveSet) * g->MoveSetsCount);
...
//create a function that gets a set and returns its string representation
void GetSetAsString(char* str, MoveSet set)
{
	...
	for (int i = 0; i < set.Length; i++)
	{
		...
		if (m.from == 0 || m.from == 25)
			sprintf_s(tempString, 100, "b");
		else
			sprintf_s(tempString, 100, "%d", m.from);
		strcat_s(finalString, 300, tempString);

		sprintf_s(tempString, 100, "-");
		strcat_s(finalString, 300, tempString);
		if (m.to == 0 || m.to == 25)
			sprintf_s(tempString, 100, "o");
		else
			sprintf_s(tempString, 100, "%d", m.to);
		strcat_s(finalString, 300, tempString);
		if (i < set.Length - 1)
			sprintf_s(tempString, 100, "X");
		else
			sprintf_s(tempString, 100, "\n");
		strcat_s(finalString, 300, tempString);
	}
	strcpy_s(str, 300, finalString);
}
...
void PlayGame(Game* g) {
	...
	while (g->BlackLeft > 0 && g->WhiteLeft > 0 && g->Turns < GetSettings()->MaxTurns)
	{
		/*if (Settings.PausePlay)
			Pause(g);*/
		g->EvalCounts = 0;
		int depth = GetSettings()->SearchDepth;
		char buf[5000];
		MoveSet bestSet;
		if (FindBestMoveSet(g, &bestSet, depth) >= 0)
		{
			if (GetSettings()->PausePlay)
			{
				PrintSet(bestSet);
				fgets(buf, 5000, stdin);
			}
...
void PlayAndEvaluate() {
	TrainerStruct* Trainer = GetTrainer();
	AiConfig untrained;
...
void StartHash(Game* g) {
	g->Hash = GameStartHash;
	//printf("%llu\n", GameStartHash);
	for (int i = 0; i < 26; i++)
	{
		short v = g->Position[i];
		int ceckCount = CheckerCount(v);
		// Zero checkers has no hash.
		for (int c = 1; c <= ceckCount; c++)
		{
			g->Hash ^= PositionHash[v >> 5][i][c];
			//printf("%llu\n", PositionHash[v >> 5][i][c]);
		}
	}
}